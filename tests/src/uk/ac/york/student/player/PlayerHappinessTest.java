...
public class PlayerHappinessTest {
    @Test
    public void increaseAndDecreaseBehavesCorrectly() {
        ...
        try (var skinManager = mockStatic(SkinManager.class);
            ...
            var metric = new PlayerHappiness();
            assertThat(metric.get()).isCloseTo(metric.getDefault(), Offset.offset(0.001f));

            metric.increase(0.2f);
            assertThat(metric.get()).isCloseTo(metric.getDefault() + 0.2f, Offset.offset(0.001f));

            metric.decrease(0.2f);
            assertThat(metric.get()).isCloseTo(metric.getDefault(), Offset.offset(0.001f));
            ...
            metric.decrease(2);
            assertThat(metric.get()).isCloseTo(PlayerHappiness.PROGRESS_BAR_MINIMUM, Offset.offset(0.001f));
        }
    }
}
